...
export function registerCeoUser({ email, password, name, region = '', mobile = '' }) {
  const db = getDb();
  ...
  if (!isPlatformLegacyCeo(id)) initCeoDb(id);
...
export function listUsers() {
  return getDb()
    .prepare(
      `SELECT id, email, name, region, mobile, role, enabled, created_at, updated_at
