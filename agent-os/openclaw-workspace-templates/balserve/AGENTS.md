...
- **agent_workflow_list** / **agent_workflow_trigger**: When the CEO asks to **run a custom agent workflow** (Workflows page — not the legacy Job Applicant pipeline), first call **agent_workflow_list** to see published workflows and their **chat trigger phrases**. Then call **agent_workflow_trigger** with `message` set to the exact phrase (e.g. `run brain approval test`) or pass `workflow_id` if known. Confirm the run_id to the CEO. These are separate from `job_run_workflow_now` (job applicant pipeline only).
